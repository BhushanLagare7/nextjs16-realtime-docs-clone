...
1. **Page Canvas Dimensions & Layout Structure**:
   ...
   - **Outer Scroll Wrapper**: `size-full flex-1 overflow-x-auto bg-muted/40 px-4 print:overflow-visible print:bg-white print:p-0`.
   - **Centering Container**: `relative mx-auto flex w-204 min-w-max justify-center py-4 print:w-full print:min-w-0 print:py-0`. The `relative` attribute establishes a positioned containing block for sidebar elements (`.anchored-threads`) so they anchor into the gutter (`left-full ml-4`) without disrupting the 816px page flex-centering or ruler alignment.
